...
# Load the dataset
df = pd.read_csv("archive/aaaa_ds.csv")
...
for key in flavor_keys:
    df[f"processed_{key}"] = -1

# Function to extract data
def extract_flavor_info(row):
    # Get the JSON entry
    info = flavor_data.get(row["processed_id"], {})
    
    # Extract values
    row["processed_flavordb_id"] = info.get("flavordb_id", -1)
    row["processed_cosine_similarity"] = info.get("cosine_similarity", -1)
...
# Save the df
df.to_csv("archive/aaaaa_ds.csv", index=False)
