...
# Load the dataset
df = pd.read_csv("archive/aaa_ds.csv")
...
for nutrient in nutrient_keys:
    df[f"processed_{nutrient}"] = -1

# Function to extract nutrition data
def extract_nutrition_info(row):
    # Get JSON entry
    info = nutrition_data.get(row["processed_id"], {})
    
    # Extract values with "processed_" prefix
    row["processed_category"] = info.get("category", "-1")
    row["processed_label"] = info.get("label", "-1")
    row["processed_weight"] = info.get("weight", -1)
    
    # Extract nutrients
    nutrients = info.get("nutrients", {})
    for nutrient in nutrient_keys:
        row[f"processed_{nutrient}"] = nutrients.get(nutrient, -1)
...
# Save the df
df.to_csv("archive/aaaa_ds.csv", index=False)
